...
**Structure Decision**: The relay/server portion now lives in its own repository. This repository should remain server-side only: `bloop-relay`, relay deployment assets, and relay release automation. The local-machine client belongs in the separate `bloop-tunnel` repository.
...
The original implementation plan described a combined relay + client repository.

That is no longer the packaging model:

- this repository is the server-side relay repository
- `bloop-relay` is the public/server release artifact here
- `bloop-client` belongs in the separate `bloop-tunnel` repository

Keep future changes aligned to that boundary.
